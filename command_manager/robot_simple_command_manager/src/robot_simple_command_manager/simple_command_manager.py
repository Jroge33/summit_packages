...
class SimpleCommandManager(RComponent):
    ...
    def load_handlers(self, handler_list=None):
        ...
        if len(self.desired_handlers) != len(self.loaded_handlers):
    ...
    def reload_handlers(self):
        pending_handlers = list(set(self.desired_handlers) - set(self.loaded_handlers))
        self.load_handlers(pending_handlers)
    ...
    def service_cmd_cb(self, msg):
        ...
        if self.command_action_server.is_active() == True:
            ...
            if is_valid == True:
                ...
                rospy.logerr('%s::service_cmd_cb: %s' % (self._node_name, ret_msg))
    ...
    def action_goal_cb(self):
        ...
        if self.command_action_server.is_active() == False:
            ...
            if is_valid == True:
                ret, ret_msg = self.send_cmd(goal.command)
                if ret != True:
                    rospy.logerr('%s::action_goal_cb: error sending the command %s' %
                                 (self._node_name, goal.command.command))
                    result = RobotSimpleCommandResult()
                    result.result.command = goal.command.command
                    result.result.success = False
                    result.result.message = ret_msg
